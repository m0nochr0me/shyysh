...
class ListView(Frame):
    def __init__(self, screen, model):
        ...
        self._session = self._tmux_server.sessions.get(
            session_id=self._pane["session_id"]
        )
    ...
    def _connect(self):
        ...
        self._session.new_window(
            f'{_conn["title"]}', attach=True, window_shell=_conn_cmd
        )
...
class ConnectionView(Frame):
    def __init__(self, screen, model):
        ...
        layout.add_widget(
            Text("User:", "user", validator=r"^([A-Za-z0-9].*)$|^$")
        )
        ...
        layout.add_widget(
            Text(
                "Port:",
                "port",
                validator=r"^(6553[0-5]|655[0-2]\d|65[0-4]\d\d|6[0-4]\d{3}|[1-5]\d{4}|[1-9]\d{0,3}|0)$|^$",
            )
        )
        layout.add_widget(Divider())
        ...
        layout.add_widget(
            CheckBox("-g", "Allow Remote Port Conn", "allow_rpc")
        )
        ...
        layout.add_widget(
            Text("Sort order:", "sort", validator=r"^(\d){0,4}$|^$")
        )
...
class ShyySH:
    ...
    def play(self, screen, scene):
        scenes = [
            Scene([ListView(screen, self._connections)], -1, name="Main"),
            Scene(
                [ConnectionView(screen, self._connections)],
                -1,
